...
class CuraEngineFanByFeaturePluginConan(ConanFile):
    ...
    def _cura_plugin_name(self):
        return "CuraEngineFanSpeedByFeatureType"
    ...
    def layout(self):
        ...
        self.cpp.build.bins = ["curaengine_plugin_fan_by_feature"]
    ...
    def requirements(self):
        ...
        self.requires("curaengine_grpc_definitions/0.3.1")
    ...
    def deploy(self):
        ext = ".exe" if self.settings.os == "Windows" else ""
        copy(self, pattern=f"curaengine_plugin_fan_by_feature{ext}", dst=self.install_folder, src=os.path.join(self.package_folder, "bin"))
